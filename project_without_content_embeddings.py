...
song_data = pd.read_csv("CODICE PAPER+DATASET/songs_final.csv", sep=',')
user_data = pd.read_csv("CODICE PAPER+DATASET/user_data_final.csv", sep=',')
...
context_embeddings = pickle.load(open('CODICE PAPER+DATASET/context_embeddings.p', 'rb'))
# walksSAT = pickle.load(open('CODICE PAPER+DATASElkT/wasSAT.p', 'rb'))
...
# user_data["arousal"] = user_data.merge(song_data, on="song_id", how="left").set_index("triple_id")
# user_data["valence"] = user_data.merge(song_data, on="song_id", how="left").set_index("triple_id")
...
def generate_embeddings(row, e_dict):
    embedding = context_embeddings[row[0]]
...
pickle.dump(song_embeddings, open('CODICE PAPER+DATASET/song_embeddings.p', 'wb'))
print("song_embeddings.p salvato.\n")

# salvataggio user embeddings
print("Salvataggio user embeddings...")
pickle.dump(user_embeddings, open('CODICE PAPER+DATASET/user_embeddings.p', 'wb'))
...
print("Salvataggio song embeddings...")
pickle.dump(song_embeddings, open('CODICE PAPER+DATASET/song_embeddings_zscore.p', 'wb'))
print("song_embeddings_zscore.p salvato.\n")

# salvataggio user embeddings con z-score
print("Salvataggio user embeddings...")
pickle.dump(user_embeddings, open('CODICE PAPER+DATASET/user_embeddings_zscore.p', 'wb'))
...
w_equi = np.concatenate((w_1), axis=None)
...
w_mix = np.concatenate((w_1_mix), axis=None)
...
print("Song_data_listened")
print(song_data_listened.head())

print(user_data_total.merge(song_data_listened, left_on="song_id", right_on="song_id", how="left")["id"].dtypes)
print("user_data_total prima della fusione")
print(user_data_total.head())
user_data_total["song_matrix_id"] = user_data_total.merge(song_data_listened, left_on="song_id", right_on="song_id", how="left")["id"]
print(user_data_total["song_matrix_id"].dtypes)
print("user_data_total dopo la fusione")
print(user_data_total.head())
...
print(user_hyperedges.columns)
print(user_test.columns.values[0])

user_test = user_test.merge(user_hyperedges, on="user", how="left").set_index("triple_id")
print("USER_TEST")
...
print(user_training.columns)
print(user_hyperedges.columns)
user_training = user_training.merge(user_hyperedges, on="user", how="left").set_index("triple_id")
